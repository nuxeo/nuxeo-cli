require('./proxy');

module.exports = () => {
  const optimist = require('yargs')
    .usage('Usage: $0 <command> [options] [args]')
    .commandDir('../commands')
    .example('$0 bootstrap operation listener')
    .example('$0 hotreload')
    .example('$0 hotreload configure')
    .example('$0 sample')
    .example('$0 studio link')
    .example('$0 studio import')
    .example('$0 studio export')
    .example('$0 sync [--src resources/ui] [--dest nuxeo/nxserver/nuxeo.war/ui]')
    .example('$0 update')
    .options('h', {
      alias: 'help',
      describe: 'Print this message, or any command help.',
      type: 'boolean'
    })
    .options('b', {
      alias: 'batch',
      describe: 'Batch mode, force non-interactive shell. Bootstrap properties are read from options `--params.<field>=<value>``',
      type: 'boolean',
      implies: 'params'
    })
    .options('params', {
      hidden: true,
      describe: 'Used in batch mode for passing parameters to generator',
      default: {}
    })
    .options('v', {
      alias: 'version',
      describe: 'Show version',
      type: 'boolean'
    })
    .options('n', {
      describe: 'Quiet - Hide welcome message',
      type: 'boolean'
    });

  const argv = optimist.argv;

  if (!argv.n) {
    console.log(require('generator-nuxeo/lib/welcome.js')('CLI'));
  }

  if (argv.version) {
    require('../lib/display_version')(console.log);
    process.exit(0);
  }

  if (argv._.length === 0) {
    optimist.showHelp();
    process.exit(argv.help ? 0 : 1);
  }
};
